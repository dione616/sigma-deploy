...
exports.login.post("/login", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var user, decoded, decodedPassword;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                if (!(req.body.email && req.body.password)) return [3 /*break*/, 2];
                return [4 /*yield*/, UserSchema_1.User.findOne({ email: req.body.email }).catch(function (err) {
                        res.send(err);
                    })];
            case 1:
                user = _a.sent();
                if (user) {
                    decoded = jsonwebtoken_1.default.verify(user.token, "" + process.env.TOKEN_SECRET);
